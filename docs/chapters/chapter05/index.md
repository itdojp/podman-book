...
_2026-07-23（JST）、Ubuntu 24.04.3 LTS（WSL2）/ rootless Podman 4.9.3、Alpine 3.20。2 stageの命令、commit、`localhost/book-qa-image:issue189`のtag、実行結果を対応付けます。container IDとorganization labelはマスクしています。_
...
- Containerfile/Dockerfile の主要命令と、運用を見据えた記述の観点（ベースイメージ、メタデータ、環境変数など）を整理しました。
- マルチステージビルドやキャッシュなど、イメージ作成を効率化する手法を扱いました。
- Buildah を用いた高度なビルド、レジストリ運用、イメージ署名・検証の基本を確認しました。
