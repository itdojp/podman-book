...
> **注意（OS設定の変更について）**
> - `/etc/subuid`・`/etc/subgid` や `sysctl` の設定変更は、ホストOSの挙動に影響します。誤設定すると開発環境や既存のコンテナ環境に影響が出る可能性があります。
...
_2026-07-23、Ubuntu 24.04.3 LTS（WSL2）/ rootless Podman 4.9.3、Alpine 3.20。rootlessとseccompが有効であること、SELinuxは当該Ubuntu環境では無効であること、`--cap-drop=all`後の`CapEff`が0であることを確認します。_
...
- コンテナセキュリティを多層防御として捉え、各レイヤーの役割（Namespace/Capabilities/Seccomp/SELinux等）を整理しました。
- Rootless実行を中心に、運用で重要になるセキュリティ機能の使い方と注意点を確認しました。
- イメージ/ランタイム/コンプライアンスの観点から、実務でのセキュリティ運用に必要な要点を扱いました。
