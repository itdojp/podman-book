...
_2026-07-23（JST）、Ubuntu 24.04.3 LTS（WSL2）/ Podman・Quadlet 4.9.3 / systemd 255。`.container`のimage・name・port・restartが生成unitへ反映されることをdry-runで確認します。撮影環境ではuser systemd busがofflineのため、稼働状態の証跡ではありません。workspace pathはマスクしています。_
...
「プロセスが動いている」≠「サービスが正常」です。適切なヘルスチェックにより、次のことが可能になります。
- 早期の問題検出
- 自動復旧の実現
- SLAの維持
...
- systemdとPodmanを統合し、コンテナを「本番のサービス」として管理する考え方を整理しました。
- 自動アップデート、バックアップ/リストア、監視とアラートなど、運用で必要になる仕組みを確認しました。
- 性能面のボトルネックを把握し、実運用で有効なチューニング観点を扱いました。

## 次に読む

- [第10章：CI/CDパイプラインの実践](../chapter10/)
- [目次に戻る](../../)
