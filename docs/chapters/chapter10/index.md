...
本章では、セキュアで効率的なCI/CDパイプラインの構築方法を学びます。ログ確認や障害時の一次切り分けは、[付録B：トラブルシューティングガイド](../../appendices/appendix-b/) を起点に整理してください。
...
_2026-07-23（JST）、GitHub Actions Web UI。公開main commit `4a7955d`のBook QA runで、status、`qa` job、2 artifactが同じ成功runに属することを確認します。accountとorganizationはマスクしています。この画面はbook QAの実例であり、後続のPodman image build例はworkflow定義とrun logを併せて検証してください。_
...
pipeline {
    ...
    post {
        always {
            // クリーンアップ - なぜ重要か
            // 注意: 共有ホスト/共有ランナーで実行すると他ジョブのイメージ等も削除され得る。
            // 専用ランナー（Podman を当該ジョブ専用ユーザーで利用）での運用を前提とする。
...
- PodmanをCI/CDに組み込み、セキュアで再現性の高いパイプラインを構築する考え方を整理しました。
- GitLab CI/CD・GitHub Actions・Jenkinsなど、代表的な基盤との統合例を確認しました。
- 自動テスト戦略とデプロイメント戦略（Blue-Green/Canary等）のポイントを扱いました。

## 次に読む

- [第11章：Kubernetesとの統合](../chapter11/)
- [目次に戻る](../../)
