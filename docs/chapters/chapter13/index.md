...
モノリシックアーキテクチャの課題は次のとおりです。
- スケーラビリティの制限
- 技術スタックの固定化
- デプロイメントリスク
- チーム間の依存性

マイクロサービスがもたらす価値は次のとおりです。
- 独立したスケーリング
- 技術選択の自由
- 障害の局所化
- チームの自律性
...
_2026-07-23（JST）、Ubuntu 24.04.3 LTS（WSL2）/ rootless Podman 4.9.3、Nginx 1.27-alpine・Alpine 3.20。database processとhealthyなAPIが同じprivate bridge network上でrunningであることを、service単位で確認します。_
...
- マイクロサービスの基本概念と、設計原則（境界・データ・依存関係）を整理しました。
- Podmanでマイクロサービス環境を構築し、サービス間通信やオーケストレーションの実装例を確認しました。
- 可観測性（分散トレーシング）とレジリエンス（サーキットブレーカー等）の観点を扱いました。

## 次に読む

- [第14章：エンタープライズ環境での活用](../chapter14/)
- [目次に戻る](../../)
