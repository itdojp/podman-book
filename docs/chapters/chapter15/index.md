...
<a id="figure-failure-diagnosis-recovery"></a>
...
より実践的なトラブルシューティング例については、[実践的トラブルシューティングガイド](../../additional/troubleshooting-guide/)を参照してください。

## まとめ

- トラブルシューティングのフレームワークと、診断ツールキットの組み立て方を整理しました。
- 起動/ネットワーク/権限などの典型的な問題に対して、切り分けと対処の進め方を確認しました。
- 高度なデバッグや自動化まで含め、運用で再現性を高めるための観点を扱いました。
