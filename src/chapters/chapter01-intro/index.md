---
title: "第1章：Podman概要と環境構築"
---

# 第1章：Podman概要と環境構築

## 学習目標
この章を読み終える頃には、以下のことができるようになります：
- PodmanとDockerの本質的な違いを説明できる
- Windows WSL2環境でPodmanを構築できる
- Podman CLIとPodman Desktopの使い分けができる
- 基本的なPodmanコマンドを実行できる

---

## 1.1 PodmanとDockerの違い

### アーキテクチャ比較

PodmanとDockerの最も重要な違いは、そのアーキテクチャにあります：

![DockerとPodmanのアーキテクチャ比較]({{ '/assets/images/diagrams/chapter01-docker-podman-architecture.svg' | relative_url }})

### 主要な違いの詳細

#### 1. **デーモンレス設計**

**Docker**:
- 常駐デーモン（dockerd）が必要
- デーモン停止で全コンテナが影響
- システムリソースを常時消費

**Podman**:
- デーモン不要で直接実行
- 各コンテナが独立して動作
- 必要時のみリソース使用

#### 2. **Rootless実行**

**Docker**:
```bash
# Docker (Root権限必要)
sudo docker run nginx
# または
sudo usermod -aG docker $USER  # docker グループ追加が必要
```

**Podman**:
```bash
# Podman (一般ユーザー権限で実行可能)
podman run nginx
```

#### 3. **セキュリティ**

| セキュリティ面 | Docker | Podman | Podmanの利点 |
|---------------|--------|--------|-------------|
| **Root権限** | 通常必要 | 不要 | 攻撃対象面の縮小 |
| **デーモン** | 常駐 | なし | プロセス脆弱性回避 |
| **SELinux** | 追加設定 | ネイティブ対応 | 強化されたアクセス制御 |

#### 4. **Pod概念**

Podmanの最大の特徴は、**Pod**（複数コンテナのグループ）をネイティブサポートしていることです：

```
Docker Compose (外部ツール)
┌─────────────────────────────────────┐
│ docker-compose.yml                  │
│  services:                          │
│    web: nginx                       │
│    db: postgres                     │
└─────────────────────────────────────┘
             │
    ┌─────────────────┐
    │ Docker Daemon   │
    └─────────────────┘
             │
┌─────────────┐  ┌─────────────┐
│ nginx       │  │ postgres    │
│ Container   │  │ Container   │
└─────────────┘  └─────────────┘

Podman Pod (ネイティブ)
┌─────────────────────────────────────┐
│ Pod: myapp                          │
│  ┌─────────────────────────────────┐ │
│  │ Pause Container (Infrastructure) │ │
│  └─────────────────────────────────┘ │
│  ┌─────────────────────────────────┐ │
│  │ nginx Container                 │ │
│  └─────────────────────────────────┘ │
│  ┌─────────────────────────────────┐ │
│  │ postgres Container              │ │
│  └─────────────────────────────────┘ │
│  Shared: Network, IPC, PID          │
└─────────────────────────────────────┘
```

### コマンド互換性

Podmanは**Docker CLI互換**を提供しています：

| 機能 | Docker | Podman | 互換性 |
|------|--------|--------|--------|
| イメージビルド | `docker build` | `podman build` | 完全互換 |
| コンテナ実行 | `docker run` | `podman run` | 完全互換 |
| イメージ管理 | `docker images` | `podman images` | 完全互換 |
| Pod管理 | なし | `podman pod` | Podman独自 |
| Compose | `docker-compose` | `podman-compose` | 互換ツール |

**Docker代替設定**:
```bash
alias docker=podman
```

---

## 1.2 Windows WSL2環境でのPodman

### WSL2アーキテクチャ理解

Windows環境でPodmanを実行する場合、WSL2を活用します：

![Windows WSL2環境図]({{ '/assets/images/diagrams/chapter01-windows-wsl2-environment.svg' | relative_url }})

### 詳細セットアップ手順

#### ステップ1: WSL2有効化

**PowerShell（管理者権限）で実行**:
```powershell
# WSL機能の有効化
dism.exe /online /enable-feature /featurename:Microsoft-Windows-Subsystem-Linux /all /norestart

# 仮想マシンプラットフォーム機能の有効化
dism.exe /online /enable-feature /featurename:VirtualMachinePlatform /all /norestart

# システム再起動後、WSL2をデフォルトに設定
wsl --set-default-version 2
```

#### ステップ2: Linuxディストリビューション選択

**Ubuntu 22.04 LTS推奨理由**:
- 最新のPodmanパッケージが利用可能
- 豊富なドキュメントとコミュニティサポート
- 企業環境での実績

```bash
# Ubuntu 22.04のインストール
wsl --install -d Ubuntu-22.04

# インストール確認
wsl --list --verbose
```

#### ステップ3: Podmanインストール

**Ubuntu内での作業**:
```bash
# パッケージリストの更新
sudo apt update && sudo apt upgrade -y

# 必要なパッケージのインストール
sudo apt install -y curl wget gnupg2 software-properties-common

# Podmanのインストール
sudo apt install -y podman

# インストール確認
podman --version
podman info
```

#### ステップ4: Rootless設定

**UID/GIDマッピング設定**:
```bash
# 現在のユーザー設定確認
cat /etc/subuid
cat /etc/subgid

# 必要に応じて設定（通常は自動設定済み）
echo "$USER:100000:65536" | sudo tee -a /etc/subuid
echo "$USER:100000:65536" | sudo tee -a /etc/subgid

# Podman初期化
podman system migrate
```

#### ステップ5: 動作確認

**基本テスト**:
```bash
# システム情報確認
podman info

# テストコンテナ実行
podman run hello-world

# イメージ一覧確認
podman images

# 実用的なテスト
podman run -d --name test-nginx -p 8080:80 nginx:alpine
curl http://localhost:8080
podman stop test-nginx
podman rm test-nginx
```

---

## 1.3 Podman CLI vs Podman Desktop

### 機能比較

#### Podman CLI
**特徴**:
- 軽量で高速
- スクリプト化が容易
- 本格的な運用に適している
- リモートサーバーでの利用

**適用場面**:
```bash
# 自動化スクリプト例
#!/bin/bash
podman build -t myapp .
podman run -d --name myapp-prod -p 80:8080 myapp
podman logs myapp-prod
```

#### Podman Desktop

**アーキテクチャ**:
```
Podman Desktop
┌─────────────────────────────────────┐
│ Windows                             │
│  ┌─────────────────────────────────┐ │
│  │ Podman Desktop (GUI)            │ │
│  │  ├─ Image管理                   │ │
│  │  ├─ Container管理               │ │
│  │  └─ Pod管理                     │ │
│  └─────────────────────────────────┘ │
│           │                         │
│  ┌─────────────────────────────────┐ │
│  │ WSL2 Backend                    │ │
│  │  podman machine → containers    │ │
│  └─────────────────────────────────┘ │
└─────────────────────────────────────┘
```

### 詳細比較表

| 側面 | Podman CLI | Podman Desktop |
|------|------------|----------------|
| **使用方法** | コマンドライン | GUI操作 |
| **リソース使用量** | 軽量 | 中程度（GUI分） |
| **学習コスト** | 高（CLI習得必要） | 低（直感的操作） |
| **自動化適性** | ◎ 高（スクリプト化容易） | △ 中（CLI併用必要） |
| **トラブルシューティング** | ◎ 高（ログ直接確認） | ○ 中（GUI経由確認） |
| **開発効率** | 熟練者向け | 初心者向け |
| **サーバー運用** | ◎ 適している | × 不適切 |

### 使い分けの指針

```
学習フェーズ別推奨
┌─────────────────────────────────────┐
│ 初心者（概念理解）                   │
│  └─ Podman Desktop推奨              │
├─────────────────────────────────────┤
│ 基本操作習得                         │
│  └─ CLI + Desktop併用               │
├─────────────────────────────────────┤
│ 実践開発                             │
│  └─ CLI中心、Desktop補助            │
├─────────────────────────────────────┤
│ 本格運用                             │
│  └─ CLI専用                         │
└─────────────────────────────────────┘
```

---

## 1.4 基本操作の確認

### コンテナライフサイクル

```
Container Lifecycle with Podman
┌─────────────┐    ┌─────────────┐    ┌─────────────┐
│ Image       │───▶│ Container   │───▶│ Running     │
│ (pull)      │    │ (create)    │    │ (start)     │
└─────────────┘    └─────────────┘    └─────────────┘
       ▲                                      │
       │           ┌─────────────┐            │
       └───────────│ Remove      │◀───────────┘
                   │ (rm)        │
                   └─────────────┘
```

### 基本コマンド体系

#### イメージ管理
```bash
# イメージ検索
podman search nginx

# イメージ取得
podman pull nginx:alpine

# ローカルイメージ一覧
podman images

# イメージ詳細情報
podman inspect nginx:alpine

# イメージ削除
podman rmi nginx:alpine

# マルチアーキテクチャビルド
podman buildx build --platform linux/amd64,linux/arm64 -t myapp:latest .
```

#### マルチアーキテクチャビルドプロセス

![マルチアーキテクチャビルドプロセス図]({{ '/assets/images/diagrams/chapter01-multi-arch-build-process.svg' | relative_url }})

#### コンテナ管理
```bash
# コンテナ実行（フォアグラウンド）
podman run -it ubuntu:22.04 /bin/bash

# コンテナ実行（バックグラウンド）
podman run -d --name web -p 8080:80 nginx:alpine

# 実行中コンテナ一覧
podman ps

# 全コンテナ一覧（停止中含む）
podman ps -a

# コンテナ停止
podman stop web

# コンテナ削除
podman rm web

# コンテナログ確認
podman logs web
```

#### Pod管理（Podman独自）

PodmanのPodは、Kubernetesと同様の概念で、複数のコンテナをグループ化して管理できます：

![Docker Compose vs Podman Pod比較図]({{ '/assets/images/diagrams/chapter01-docker-compose-vs-podman-pod.svg' | relative_url }})

```bash
# Pod作成
podman pod create --name myapp -p 8080:80

# Pod内でコンテナ実行
podman run -d --pod myapp nginx:alpine

# Pod一覧
podman pod ls

# Pod停止
podman pod stop myapp

# Pod削除
podman pod rm myapp
```

### Podman Composeワークフロー

![Podman Compose ワークフロー図]({{ '/assets/images/diagrams/chapter01-podman-compose-workflow.svg' | relative_url }})

### 実践的なワークフロー例

#### Web開発環境の構築
```bash
# 1. 開発用Podの作成
podman pod create --name devenv -p 3000:3000 -p 5432:5432

# 2. データベースコンテナ（PostgreSQL）
podman run -d --pod devenv \
  --name db \
  -e POSTGRES_PASSWORD=devpass \
  -e POSTGRES_DB=myapp \
  postgres:13

# 3. Webアプリケーションコンテナ（Node.js）
podman run -d --pod devenv \
  --name web \
  -v $(pwd):/app:Z \
  -w /app \
  node:22 \
  npm start

# 4. 状態確認
podman pod ps
podman ps --pod

# 5. ログ確認
podman logs web
podman logs db

# 6. 開発終了時のクリーンアップ
podman pod stop devenv
podman pod rm devenv
```

---

## 1.5 トラブルシューティング基礎

### 一般的な問題と対処法

#### 1. **権限エラー**
```bash
# 問題: Permission denied
Error: cannot open /var/lib/containers/storage/overlay/...

# 原因: SELinuxのコンテキスト問題
# 対処: :Z オプションでボリュームマウント
podman run -v /host/path:/container/path:Z image
```

#### 2. **ポート競合**
```bash
# 問題: Port already in use
Error: cannot listen on the TCP port: listen tcp :8080: bind: address already in use

# 対処: 使用中ポートを確認
ss -tlnp | grep 8080
podman ps --format "table {{.Names}}{{.Ports}}"
```

#### 3. **イメージ取得失敗**
```bash
# 問題: Registry unreachable
Error: error pulling image "nginx": unable to pull...

# 対処: レジストリ設定確認
podman info --format json | jq .registries

# 代替レジストリ使用
podman pull docker.io/nginx
```

#### 診断コマンド集
```bash
# システム情報
podman info

# ストレージ使用量
podman system df

# システムイベント
podman events

# デバッグ実行
podman --log-level debug run nginx

# システムクリーンアップ
podman system prune -a
```

---

## まとめ

この章では、Podmanの概要と環境構築について学習しました：

### 📚 学習した内容
- **PodmanとDockerの違い**: デーモンレス、Rootless、Pod概念
- **WSL2環境構築**: Windows上でのLinuxコンテナ実行
- **CLI vs Desktop**: 用途に応じた使い分け
- **基本操作**: イメージ・コンテナ・Pod管理

### 🎯 次章への準備
第2章では、実際の基本操作を通じて**Podmanの内部動作**を理解し、**より実践的な使用方法**を学習します。

---

**理解度確認**
□ Podmanのデーモンレス設計の利点を説明できる  
□ WSL2環境でPodmanをセットアップできる  
□ 基本的なコンテナ操作コマンドを実行できる  
□ Pod概念の意義を理解している
